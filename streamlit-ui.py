...
if mode_choice == "inference":
    ...
    if st.button("Inference"):
        new_row = [[culmen_length_mm, culmen_depth_mm, flipper_length_mm, body_mass_g]]
        ...
        new_sample = [culmen_length_mm, culmen_depth_mm, flipper_length_mm, body_mass_g]
        fig = util.visual_plot(X, y, new_sample, label_choice_1, label_choice_2)
        ...
        st.sidebar.image(
            image,
            caption='Does this not look like your penguin? Update our model and choose "Train".',
        )
else:
    st.header("Training")
    st.write(
    ...
    if st.button("Train"):
        with st.spinner("Training the model..."):
            ...
            df, X, y = util.data_pipeline("data/penguins_size_cleaned.csv")
            model, score = model_train.train(X, y, hyper_tuning_choice, 5)
