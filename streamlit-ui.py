...
if mode_choice == "inference":
    ...
    st.write("In this mode, you can input the four required characteristic input number to make a prediction on what species of penguin they come from.")
    ...
    if st.button('Inference'):
        result = classifier.predict([[culmen_length_mm,culmen_depth_mm,flipper_length_mm, body_mass_g]])
        st.write(f'Predicted Penguin Species:{result}')
else:
    st.header("Training")
    st.write("In this mode, you will be able to supply a new training data point, and retrain the model.")
    culmen_length_mm = st.number_input('culmen_length_mm')
    culmen_depth_mm = st.number_input("culmen_depth_mm")
    flipper_length_mm = st.number_input("flipper_length_mm")
    body_mass_g = st.number_input("body_mass_g")
    ...
    if st.button('Train'):
        with st.spinner('Training the model...'):
            new_row = [Species, culmen_length_mm, culmen_depth_mm, flipper_length_mm, body_mass_g]
            util.add(new_row)
            model, score = model_train.train("data/penguins_size.csv", hyper_tuning_choice, 5)
